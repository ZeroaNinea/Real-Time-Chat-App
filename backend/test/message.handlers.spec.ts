...
describe('Auth Socket Handlers', () => {
  ...
  it('should not allow user to reply their own message', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token },
      transports: ['websocket'],
    });
  ...
  it('should return a server error during replying a message', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token3 },
      transports: ['websocket'],
    });
  ...
  it('should reply in the private chat', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token2 },
      transports: ['websocket'],
    });
  ...
  it('should return message is not found if message ID is incorrect during replying in the private chat', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token2 },
      transports: ['websocket'],
    });
  ...
  it('should return members are not found when uses the fake private chat room 2 to rply the private message', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token2 },
      transports: ['websocket'],
    });
    ...
    clientSocket.on('connect', () => {
      ...
      clientSocket.on('roomJoined', async ({ chatId }) => {
        ...
        clientSocket.emit(
          ...
          (err: { error: string }) => {
            expect(err.error).to.equal('Members are not found.');
            clientSocket.disconnect();
            done();
          }
        );
  ...
  it('should return you are banned if user 2 tries to reply the private message', (done) => {
    const clientSocket = Client(address, {
      auth: { token: token2 },
      transports: ['websocket'],
    });
    ...
    clientSocket.on('connect', () => {
      ...
      clientSocket.on('roomJoined', async ({ chatId }) => {
        ...
        clientSocket.emit(
          ...
          (err: { error: string }) => {
            expect(err.error).to.equal(
              'You cannot reply in this chat (ban restriction).'
            );
            clientSocket.disconnect();
            done();
          }
